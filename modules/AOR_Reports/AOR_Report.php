...
class AOR_Report extends Basic {
	...
	function build_report_query_where($query = array()) {
		...
		//Ruvin - #3431
		if (!($condition->operator == "Ignore" AND $condition->value_type == "Field")) {

			$sql = "UPDATE aor_fields SET display='1' WHERE aor_report_id = '" . $this->id . "' ";
			$result = $this->db->query($sql);
		...
		if ($beanList[$this->report_module]) {
			...
			while ($row = $this->db->fetchByAssoc($result)) {
				...
				//Ruvin - #3431
				if ($condition->operator == "Ignore" AND $condition->value_type == "Field") {
					$sql = "UPDATE aor_fields SET display='0' WHERE aor_report_id = '" . $this->id . "' AND field = '" . $condition->field . "' ";
					$result = $this->db->query($sql);
				}
// - RR
